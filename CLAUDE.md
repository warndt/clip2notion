...
- ⚠️ **The binding limit is the MCP client's patience, not Netlify's 10s.** Measured 2026-08-14: every request that reached the function returned successfully, the slowest at 5.4s — yet the caller still reported "the connector's server isn't responding" for calls in that range. The failures never arrived at all. claude.ai gives up well before Netlify does, so the wait budgets target roughly **3 seconds**, not ten. Making a response *complete* is not the same as making it *arrive*.
